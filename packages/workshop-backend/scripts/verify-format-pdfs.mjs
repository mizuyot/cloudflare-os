...
function clientFromGadget(name) {
  const bytes = readFileSync(join(pkgRoot, "format-blueprints", `${name}.gadget`));
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const metaLen = view.getUint32(12);
  const doc = new Y.Doc();
  Y.applyUpdateV2(doc, gunzipSync(bytes.subarray(PREFIX + metaLen)));
