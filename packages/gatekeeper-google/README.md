...
- **Connections:** when a user connects Google (or signs in and later connects it), the scopes for
  the selected resources (Gmail, Docs, Sheets, Drive folders, Calendar, or BigQuery — see below) are requested so
...
You'll need to enable the Google APIs that you want to use. Currently supported: Gmail, Google Docs, Google Sheets, Google Drive folders, Google Calendar, and BigQuery.
...
The Google Drive API powers the Docs / Sheets / folder pickers, and also searches and reads files when a Drive folder is connected. Document edits still go through the Google Docs API, and single-spreadsheet reads go through the Google Sheets API.
...
- `drive.readonly` so a connected Drive folder can be searched and its files read (including export of Google Docs / Sheets / Slides).
...
5. Choose a Google resource type: Gmail, Google Doc, Google Spreadsheet, Drive Folder, Google Calendar, or BigQuery.
